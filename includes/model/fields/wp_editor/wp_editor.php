 ...
 *
 */
if ( ! class_exists( 'AGWP_TABBY_wp_editor' ) ) {
  class AGWP_TABBY_wp_editor extends AGWP_TABBYP {
    ...
    public function render() {
      ...
      echo ( agm_wp_editor_api() ) ? '<div class="agm-wp-editor" data-editor-settings="'. esc_attr( wp_json_encode( $editor_settings ) ) .'">' : '';
    ...
    public function enqueue() {
      ...
      if ( agm_wp_editor_api() && function_exists( 'wp_enqueue_editor' ) ) {
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var agm_media_buttons = '. wp_json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( agm_wp_editor_api() && class_exists( '_WP_Editors') ) {

        $defaults = apply_filters( 'agm_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );

        $setup = _WP_Editors::parse_settings( 'agm_wp_editor', $defaults );

        _WP_Editors::editor_settings( 'agm_wp_editor', $setup );
