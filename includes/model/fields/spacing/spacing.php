 ...
 *
 */
if ( ! class_exists( 'AGWP_TABBY_spacing' ) ) {
  class AGWP_TABBY_spacing extends AGWP_TABBYP {
    ...
    public function render() {
      ...
      $is_unit = ( ! empty( $unit ) ) ? ' agm--is-unit' : '';
      ...
      if ( ! empty( $args['all'] ) ) {
        ...
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[all]' ) ) .'" value="'. esc_attr( $value['all'] ) .'"'. esc_attr($placeholder) .' class="agm-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( $unit ) ? '<span class="agm--label agm--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
        ...
        foreach ( $properties as $property ) {
          ...
          echo '<div class="agm--input">';
          echo ( ! empty( $args[$property.'_icon'] ) ) ? '<span class="agm--label agm--icon">'. wp_kses_post($args[$property.'_icon']) .'</span>' : '';
          echo '<input type="number" name="'. esc_attr( $this->field_name( '['. $property .']' ) ) .'" value="'. esc_attr( $value[$property] ) .'"'. esc_attr($placeholder) .' class="agm-input-number'. esc_attr( $is_unit ) .'" step="any" />';
          echo ( $unit ) ? '<span class="agm--label agm--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
          echo '</div>';
