...
if ( ! class_exists( 'AGWP_TABBY_taxonomy' ) ) {
  class AGWP_TABBY_taxonomy extends AGWP_TABBYP {
    ...
    public function render() {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if ( ! empty( $args['ajax'] ) ) {
          ...
          $args['settings']['data']['nonce'] = wp_create_nonce( 'agm_chosen_ajax_nonce' );
        ...
        $chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' agm-chosen-sortable' : '';
        $chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' agm-chosen-ajax' : '';
        $placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="'. wp_kses_post( $args['placeholder'] ) .'"' : '';
        $field_class      = ( $args['chosen'] ) ? ' class="agm-chosen'. esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) .'"' : '';
        ...
        if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
          ...
          if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
            ...
            echo '<select name="'. esc_attr($field_name) .'" class="agm-hide-select hidden"'. esc_attr($multiple_attr) . wp_kses_post($field_attr) .'>';
