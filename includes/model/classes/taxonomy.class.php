 ...
 *
 */
if ( ! class_exists( 'AGWP_TABBY_Tax_Options' ) ) {
  class AGWP_TABBY_Tax_Options extends AGWP_TABBY_Abstract{
    ...
    public function render_taxonomy_form_fields( $term ) {
      ...
      $errors    = ( ! empty( $term_id ) ) ? get_term_meta( $term_id, '_agm_errors_'. $this->unique, true ) : array();
      $errors    = ( ! empty( $errors ) ) ? $errors : array();
      $class     = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_term_meta( $term_id, '_agm_errors_'. $this->unique );
      }

      wp_nonce_field( 'agm_taxonomy_nonce', 'agm_taxonomy_nonce'. $this->unique );

      echo '<div class="agm agm-taxonomy agm-show-all agm-onload agm-taxonomy-'. esc_attr( $classname ) .'-fields '. esc_attr( $class ) .'">';
      ...
      foreach ( $this->sections as $section ) {
        ...
        if ( $taxonomy === $this->taxonomy ) {
          ...
          echo ( $section_title || $section_icon ) ? '<div class="agm-section-title"><h3>'. esc_attr($section_icon) . esc_html($section_title) .'</h3></div>' : '';
          ...
          if ( ! empty( $section['fields'] ) ) {
            foreach ( $section['fields'] as $field ) {
              ...
              AGWP_TABBY::field( $field, $this->get_meta_value( $field, $term_id ), $this->unique, 'taxonomy' );
    ...
    public function save_taxonomy( $term_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'agm_taxonomy_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';
      $taxonomy = ( ! empty( $_POST[ 'taxonomy' ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ 'taxonomy' ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'agm_taxonomy_nonce' ) ) {
        return $term_id;
      }
      ...
      $data = apply_filters( "agm_{$this->unique}_save", $data, $term_id, $this );

      do_action( "agm_{$this->unique}_save_before", $data, $term_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_term_meta( $term_id, '_agm_errors_'. $this->unique, $errors );
        }

      }

      do_action( "agm_{$this->unique}_saved", $data, $term_id, $this );

      do_action( "agm_{$this->unique}_save_after", $data, $term_id, $this );
